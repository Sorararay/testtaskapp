...
func FindTask(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Task, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"task\" where \"id\"=$1", sel,
	)
...
func (o *Task) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(taskPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert task, could not build conflict column list")
			}

			conflict = make([]string, len(taskPrimaryKeyColumns))
			copy(conflict, taskPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"task\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert task")
	}
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"task\" where \"id\"=$1 limit 1)"
